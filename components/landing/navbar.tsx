...
export function Navbar() {
  ...
  return (
    <header className={cn("fixed left-0 right-0 top-0 z-50 transition-all duration-500", scrolled ? "border-b border-border/50 bg-background/70 backdrop-blur-2xl" : "bg-transparent", hidden ? "-translate-y-full" : "translate-y-0")}>
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Magnetic strength={0.2}>
          <Link href="/" className="group flex items-center gap-3">
            ...
            <span className="text-xl font-semibold tracking-tight">Flowcut</span>
          </Link>
        </Magnetic>
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <Magnetic key={link.href} strength={0.15}>
              <Link href={link.href} className="group relative px-4 py-2 text-sm text-muted-foreground transition-colors hover:text-foreground">
                {link.label}
